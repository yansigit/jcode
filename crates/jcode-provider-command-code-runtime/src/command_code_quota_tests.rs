...
fn command_code_quota_parses_wrapped_windows() {
    let parsed = parse_credits(
        &json!({"windowLimits": {"fiveHour": {"cap": 100, "used": 25, "resetAt": "later"}, "weekly": {"limit": 500, "usage": 50}}, "credits": 3}),
    );
...
fn failover_sticks_short_429_and_rotates_long_pre_stream() {
    ...
    assert_eq!(
        handle_command_code_error_failover(
            "a",
            429,
        ...
        CommandCodeFailoverAction::StickWait {
    ...
    assert_eq!(
        ...
        CommandCodeFailoverAction::Rotate {
            previous_account: "a".into(),
            next_account: "b".into()
        }
    );
    assert_eq!(
