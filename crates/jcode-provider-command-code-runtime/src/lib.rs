...
impl CommandCodeProvider {
    ...
    pub fn generate_request(
        ...
        let body = json!({
            "model": *self.model.read().map_err(|_| anyhow::anyhow!("model poison"))?,
            "messages": messages,
            "params": {"stream": true},
        });
        Ok(self
...
impl Provider for CommandCodeProvider {
    ...
    async fn complete(
        ...
        let request = self.generate_request(messages, tools, system)?;
        ...
        if !status.is_success() {
            ...
            anyhow::bail!("Command Code generate failed: {} (head: {})", status, head);
        }
            ...
            for line in line.split('\n').map(str::trim).filter(|l| !l.is_empty()) {
                ...
                match decode_record_line(line) {
                    ...
                    GenerateRecord::Error(message) => {
                        let _ = tx
                            .send(Ok(StreamEvent::Error { message, retry_after_secs: None }))
                            .await;
                        return;
                    }
